...
layout(binding=0, rg16f) restrict writeonly uniform image2D normal_height;

#define rads(x) (x * 3.1415926535897932384626433832795 / 180)

#define height_mapping(x) (0.95 + 0.15 * x)

#define atan2(y, x) ((0. == x) ? sign(y) * rads(90) : atan(y, x))
...
{
	return fract(sin(dot(st * rads(180 / 8192.), vec2(12.9898, 78.233))) * 43758.5453123);
}

float random3D(in vec3 st)
{
	return random2D(st.xy * (1. + log(st.z + 10.)));
}
...
float g(float ro, float v, float radius)        // crater height
{
	float _ro = ro / radius;
	float pi_v = rads(180) - v;
	return -(cos(pi_v * _ro * _ro + v) + 1.0);
}

vec2 gdiff(vec2 ro, float v, float radius)      // derivative of crater height (g')
{
	float _ro = length(ro) / radius;
	float pi_v = rads(180) - v;
	return pi_v * _ro * sin(pi_v * _ro * _ro + v) * normalize(ro);
}
...
float calculateHeightAndNormal(in float rnd1, in float rnd2, in float rnd3,
								...
								out vec3 normal)
{
	// intersection points of triangle side and line on opposite vertex and current point (wxmaxima form)
	// v1x:0$ v1y:0$ v2x:0$ v2y:1$ v3x:1$ v3y:0.5$
	// ratsimp(solve([(x-v1x)/(vx-v1x)=(y-v1y)/(vy-v1y),(x-v3x)/(v2x-v3x)=(y-v3y)/(v2y-v3y)],[x,y]));
	float rnd23 = 0.;
	float t1 = 0.;
...
float getDissolve(in float Count, in int Level, in float Multiplier)
{
	float dissolve = int(Count) / Count;
	if (Level >= int(Count))
	{
		dissolve = clamp(Level + 1. - Count, 0., 1.);
		dissolve = cos(rads(90) * dissolve);
	}
	float lvl = 1;//cos(0.5 * M_PI * Level / Count);
	int shift = min(16, 1 + int(Count));
	if (Level >= Count - shift)
	{
		lvl = cos(rads(90) * clamp((Level - (Count - shift)) / shift, 0., 1.));
				...
				out int hsector_index, out int vindex)
{
	...
	vang = atan2(-pos.z, length(pos.xy));                         // vertical angle
	...
	float hang = atan2(pos.y, pos.x) + rads(180);                 // angle of pos in OXY plane
...
vec4 height_map(in vec3 Pos, in int Start, in float Count, in float Multiplier)
{
	...
	for (int level = 0; level < 1 + int(Count); level ++)
	{
		...
		float rnd1 = 1. - random3D(pos1);
		float rnd2 = 1. - random3D(pos2);
		float rnd3 = 1. - random3D(pos3);
		...
		float dissolve = getDissolve(Count, level, Multiplier);
		...
		if (intPow >= 4)
		{
			...
			if (random2D(M_PI * vec2(hsector_index, vindex)) < crater_chance)
			{
				...
				float radius = r - max(ofs.x, ofs.y) * 1.3;                     // crater radius (crater shouldn't get out of sector)
				float nmldirlen = length(nmldir);                               // length of vector should be less than radius
				if (nmldirlen < radius)                                         // if current point is inside circle
				{
					...
					vec3 unscaled = vec3(0., gdiff(nmldir, shape, radius) * scale * dissolve);  // gdiff - производная g (g')
					nml = normalize(local_coords) - unscaled;
					nml.xz = rotate2D(nml.xz, center.y);
					nml.xy = rotate2D(nml.xy, center.x);                        // transform normal back to "global" coordinates
					height += 0.3 * g(nmldirlen, shape, radius) / intPow;       // g is height of crater, is added to ground height
	...
	avgnormal1 = 2. * (normalize(avgnormal1) - pos) + pos;                      // weight of normals
	avgnormal2 = 32. * (normalize(avgnormal2) - pos) + pos;
...
void main(void)
{
	float x = gl_GlobalInvocationID.x / float(imageSize(normal_height).x);
	float y = gl_GlobalInvocationID.y / float(imageSize(normal_height).y);

	vec3 pos = vec3(x / dx, y / dy, 1);
	vec4 result = height_map(normalize(pos), 1, 5, 1.);

	imageStore(normal_height, ivec2(gl_GlobalInvocationID), result);
}
