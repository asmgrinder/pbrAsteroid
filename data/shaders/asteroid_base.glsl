...
#define PHI 16.1803398874989484820459

float random3D(in vec3 p)
{
	vec2 arg = (p.xy + PHI) * (7.0 + log(p.z + 119.0));
	return fract(sin(dot(arg * rads(180) / 1021., vec2(12.9898, 78.233))) * 43758.5453123);
}
...
void recognizeIcoTriangle(in vec3 Pos, out vec3 localPos, out vec3 localNormal, out float rotZang, out float rotYang, out vec2 rotYdisp)
{
	...
	rotYang = 0.0;											// remember transform to be able to make inverse one
...
{   // recognize which triangle in the triangle grid current point belongs to
	...
	vec2 frac = vec2(0.0);
	float xpow2 = intPow * (1.0 - localpos.x / ICO_POINT1_X);
	float xint = trunc(xpow2);
	frac.x = clamp(xpow2 - xint, 0.0, 1.0);
	...
	if (frac.x > frac.y * 2.0					// check if grid triangle is upside down
		|| frac.x > (1.0 - frac.y) * 2.0)
	{
		yint   += (frac.y < 0.5) ? -1.0 :  0.0;
		frac.y += (frac.y < 0.5) ?  0.5 : -0.5;
		frac.x = 1.0 - frac.x;

		cr1 = vec2(xint + 1.0, yint);
		cr2 = vec2(xint + 1.0, yint + 1.0);
		cr3 = vec2(xint, yint + 1.0);
	}
...
{
	float zSign = (localpos.z >= 0.0) ? 1.0 : -1.0;
	vec2 _pos = vec2(ICO_POINT1_X * (1.0 - cr.x / intPow),
						ICO_POINT1_Y * (2.0 * cr.y + cr.x - intPow) / intPow);
	pos = vec3(_pos, (1.0 - _pos.x * (1.0 - ICO_POINT0_Z) / ICO_POINT1_X) * zSign);
								...
								out vec3 normal)
{
	...
	float rnd23 = 0.0;
	float t1 = 0.0;
	float divider1 = 2.0 * frac.y + frac.x;
	if (0.0 != divider1)
	{
		vec2 ip1 = vec2(2.0 * frac.x, 2.0 * frac.y) / divider1;
		rnd23 = smoothstep(0.0, 1.0, 1.0 - ip1.x) * (rnd2 - rnd3) + rnd3;
		float lip1 = length(ip1);
		lip1 = (0.0 == lip1) ? 1.0 : lip1;
		t1 = clamp(length(frac) / lip1, 0.0, 1.0);
	}
	float rnd13 = 0.0;
	float t2 = 0.0;
	float divider2 = frac.x - 2.0 * frac.y + 2.0;
	if (0 != divider2)
	{
		vec2 ip2 = vec2(2.0 * frac.x, frac.x) / divider2;
		rnd13 = smoothstep(0.0, 1.0, 1.0 - ip2.x) * (rnd1 - rnd3) + rnd3;
		float lip2 = length(ip2 - vec2(0.0, 1.0));
		lip2 = (0.0 == lip2) ? 1.0 : lip2;
		t2 = clamp(length(frac - vec2(0.0, 1.0)) / lip2, 0.0, 1.0);
	}
	float rnd12 = 0.0;
	float t3 = 0.0;
	float divider3 = 2.0 * frac.x - 2.0;
	if (0 != divider3)
	{
		vec2 ip3 = vec2(0.0, frac.x - 2.0 * frac.y) / divider3;
		rnd12 = smoothstep(0.0, 1.0, 1.0 - ip3.y) * (rnd1 - rnd2) + rnd2;
		float lip3 = length(ip3 - vec2(1.0, 0.5));
		lip3 = (0.0 == lip3) ? 1.0 : lip3;
		t3 = clamp(length(frac - vec2(1.0, 0.5)) / lip3, 0.0, 1.0);
	}

	float h1 = smoothstep(0.0, 1.0, 1.0 - t1) * (rnd1 - rnd23) + rnd23;
	float h2 = smoothstep(0.0, 1.0, 1.0 - t2) * (rnd2 - rnd13) + rnd13;
	float h3 = smoothstep(0.0, 1.0, 1.0 - t3) * (rnd3 - rnd12) + rnd12;

	float height = outside_crater * (h1 + h2 + h3) / (8.0 * intPow);

	float n1 = 6.0 * t1 * (1.0 - t1) * (rnd1 - rnd23);
	float n2 = 6.0 * t2 * (1.0 - t2) * (rnd2 - rnd13);
	float n3 = 6.0 * t3 * (1.0 - t3) * (rnd3 - rnd12);
...
float getDissolve(in float LastLevel, in int Level, in float Multiplier)
{
	...
	{
		dissolve = clamp(1.0 + Level - LastLevel, 0.0, 1.0);
	...
	{
		lvl = cos(rads(90) * clamp((Level - (LastLevel - shift)) / shift, 0.0, 1.0));
		lvl = lvl * lvl * 1.05;
	}
	dissolve *= lvl * (1.0 - Multiplier) + Multiplier;
...
vec4 height_map(in vec3 Pos, in int Start, in float Count, in float Multiplier)
{
	...
	vec3 avgnormal1 = vec3(0.0);
	vec3 avgnormal2 = vec3(0.0);
	float height = 0.5;
	float outside_crater = 1.0;
	...
	for (int level = 0; level < 1 + int(Count); level ++)
	{
		...
		vec3 pos1 = vec3(0.0), pos2 = vec3(0.0), pos3 = vec3(0.0);
		vec3 nmldisp1 = vec3(0.0), nmldisp2 = vec3(0.0), nmldisp3 = vec3(0.0);
		...
		float rnd1 = 1.25 * random3D(pos1);
		float rnd2 = 1.25 * random3D(pos2);
		float rnd3 = 1.25 * random3D(pos3);

		vec3 partial_normal = vec3(0.0);
		...
		float strength = (0 == level) ? 2.0 : 1.0;
		...
		if (level > 1 && intPow >= 4 && Count > 0)
		{
			...
			float levelKoef = clamp(level / Count, 0.0, 1.0);                     // level -> 0..1
			float crater_chance = smoothstep(levelKoef, 0.0, 1.0) * 0.3 + 0.1;    // chance of crater present in sector
			if (random2D(rads(180) * vec2(hsector_index, vindex)) < crater_chance)
			{
				float signvang = (vang >= 0.0) ? 1.0 : -1.0;
				...
				float t = cos(rads(90) * abs(vsector_index / (vcount - 1.0)));
				if (0 != scale.x && 0 != scale.y && t > 0)
				{
					...
					vec2 ofs = 0.3 * r * (0.2 + 0.8 * vec2(random2D(center + 13.0), random2D(center + 37.0)));// displacement from center of sector
					...
					if (radius > 0.0 && nmldirlen * 0.96 < radius)                  // if current point is inside circle
					{
						...
						float roughness = 0.5 + 0.3 * smoothstep(level / Count, 0.0, 1.0);// mark place outside crater for better heights mixing
						float t = smoothstep(1.0 - clamp(nmldirlen / radius, 0.8, 1.0) - 0.8, 0.0, 1.0);
						outside_crater = min(outside_crater, t * (1.0 - roughness) * 5.0 + roughness);
					}
				}
			}
		}
